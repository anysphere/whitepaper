...
Most existing metadata private messaging systems, such as Pung or Addra, assume that users know beforehand whom they wish to talk to and had a prior key exchange through another channel. 
Our messaging application needs a metadata-secure mechanism for the critical exchange itself. 
In other words, if Alice knows Bob's public key, then Alice should be able to send an ``invitation" to Bob. Bob must be able to retrieve this invitation from the server and complete a key exchange with Alice. The server should not learn any metadata of this exchange. We call this process ``trust establishment".
...
In the following two methods, we assume that Alice and Bob's daemons have generated a Curve25519 key exchange key pair $kx = (kx^P, kx^S)$. They aim to inform each other of their $kx^P$ to derive a shared secret $sk$.
