...
Note that we use the key exchange public key $\kx^P$ as the sole root of trust of a user's identity. This is intentional: using more human-readable identifiers like username or email in the publicID opens the possibility of social engineering attacks, which we would like to avoid. Additionally, including usernames in the publicID would create linkability between different communications, potentially compromising metadata privacy. By using only cryptographic keys as identifiers, we ensure that user identities remain pseudonymous and unlinkable across different interactions.
